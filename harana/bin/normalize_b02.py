...
def main():
    ...
    for items in tqdm(dataset):
        ...
        write_hdf5(
            os.path.join(args.dumpdir, f"{utt_id}.h5"),
            "mcep",
            scaler["mcep"].transform(mcep).astype(np.float32),
        )
        write_hdf5(
            os.path.join(args.dumpdir, f"{utt_id}.h5"),
            "bap",
            scaler["bap"].transform(bap).astype(np.float32),
        )
        write_hdf5(
            os.path.join(args.dumpdir, f"{utt_id}.h5"),
            "lf0",
            scaler["lf0"].transform(lf0).astype(np.float32),
        )
        write_hdf5(
            os.path.join(args.dumpdir, f"{utt_id}.h5"),
            "lft",
            scaler["lft"].transform(lft).astype(np.float32),
        )
        write_hdf5(
            os.path.join(args.dumpdir, f"{utt_id}.h5"),
            "spk_emb",
            scaler["spk_emb"].transform(emb).astype(np.float32),
        )
